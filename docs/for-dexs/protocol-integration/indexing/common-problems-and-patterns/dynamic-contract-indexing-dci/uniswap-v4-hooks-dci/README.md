...
The **Hooks DCI (Dynamic Contract Indexer)** is Tycho's specialized indexing system for **all Uniswap V4 hooks**. It extends the standard DCI with capabilities designed specifically for hooks, including automatic balance tracking, sophisticated entrypoint generation, and optional external metadata collection.

The Hooks DCI is required for indexing **all** Uniswap V4 hooks, regardless of where they store liquidity. It provides a complete solution with sensible defaults that work out-of-the-box for most hooks, and optional extension points for hooks with advanced requirements.
...
#### Why Hooks DCI Exists <a href="#why-hooks-dci-exists" id="why-hooks-dci-exists"></a>

The standard DCI works well for self-contained protocols, but Uniswap V4 hooks require:

* **V4-specific entrypoint generation** with custom swap encoding and state overrides
...
See [Background & Concepts](http://localhost:63342/markdownPreview/1939900721/markdown-preview-index-n7eo3ou6oo3rv58c33kbuum7bt.html#background--concepts) below for detailed explanations of hook types and architecture.
...
The Hooks DCI processes hooks with **swap permissions** (beforeSwap and/or afterSwap), as these are the ones that manage liquidity and affect swap behavior.
...
#### High-Level System Diagram <a href="#high-level-system-diagram" id="high-level-system-diagram"></a>
...
For increased performance, the external data collection was split into a three-layer architecture:
...
* Enriching metadata from block balances (internal liquidity fallback)
...
#### Internal vs External Liquidity Paths <a href="#internal-vs-external-liquidity-paths" id="internal-vs-external-liquidity-paths"></a>
...
#### Metadata Collection System <a href="#key-concepts" id="key-concepts"></a>
...
#### 4.3 Hook Orchestrators <a href="#id-43-hook-orchestrators" id="id-43-hook-orchestrators"></a>
...
#### 4.4 Entrypoint Generation <a href="#id-44-entrypoint-generation" id="id-44-entrypoint-generation"></a>
