...
1. Declare your executor's contract name and constructor arguments in <a href="https://github.com/propeller-heads/tycho-indexer/blob/main/crates/tycho-execution/config/executor_deployments.json" target="_blank" rel="noopener noreferrer">`crates/tycho-execution/config/executor_deployments.json`</a>, keyed by chain and protocol. This file is the single source of truth for executor constructor arguments and later the deployment script reads the same entries.
2. Export the runtime bytecode with the helper script in <a href="https://github.com/propeller-heads/tycho-indexer/blob/main/crates/tycho-execution/contracts/scripts/export-runtime-bytecode.js" target="_blank" rel="noopener noreferrer">crates/tycho-execution/contracts/scripts/export-runtime-bytecode.js</a> (see the <a href="https://github.com/propeller-heads/tycho-indexer/blob/main/crates/tycho-execution/contracts/scripts/README.md" target="_blank" rel="noopener noreferrer">README</a> for instructions on how).
3. Copy `YourExecutor.runtime.json` file to <a href="https://github.com/propeller-heads/tycho-indexer/tree/main/protocols/testing/fixtures" target="_blank" rel="noopener noreferrer">`protocols/testing/fixtures/`</a>.
4. Import the file in <a href="https://github.com/propeller-heads/tycho-indexer/blob/main/protocols/testing/src/execution.rs" target="_blank" rel="noopener noreferrer">protocols/testing/src/execution.rs</a> and add the corresponding entry to the `EXECUTOR_MAPPING`.

{% hint style="danger" %}
#### Block Compatibility Requirements

The `TychoRouterV3` requires post-Cancun blocks for execution. Testing must use block numbers after the Cancun upgrade.
{% endhint %}
...
**Step 2: Run a local Postgres test database using docker-compose.**
...
Select `range` or `full` depending on your test mode. The test runner compiles the Substreams package's wasm file for you before it packs the `.spkg`, so you do not need to build it yourself.

{% hint style="info" %}
**Building the wasm yourself**

Driving a package directly with `substreams run` or `substreams gui` still needs a compiled binary — see the [implementation guide](indexing/2.-implementation.md):

```bash
cd protocols/substreams/<package-name>
...
# Full tests
--initial-block    # Start block (default: the lowest initialBlock in the substreams config)
...
```bash
git clone git@github.com:propeller-heads/tycho.git
cd tycho
```
