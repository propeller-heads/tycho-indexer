...
/// Encodes a swap on BopAMM (Bebop's on-chain PMM).
///
...
#[derive(Clone)]
pub struct BopAMMSwapEncoder {
    executor_address: Bytes,
}

impl SwapEncoder for BopAMMSwapEncoder {
    fn new(
        executor_address: Bytes,
        chain: Chain,
        ...
        if chain != Chain::Ethereum {
            return Err(EncodingError::FatalError(
                "BopAMM swaps are only supported on Ethereum".to_string(),
            ));
        }
...
mod tests {
    ...
    #[test]
    fn test_encode_bopamm_weth_usdc() {
        ...
        write_calldata_to_file("test_encode_bopamm_weth_usdc", hex_swap.as_str());
    }

    #[test]
    fn test_encode_bopamm_usdc_weth() {
        ...
        write_calldata_to_file("test_encode_bopamm_usdc_weth", hex_swap.as_str());
    }

    #[test]
    fn test_encoder_rejects_non_ethereum_chain() {
        let result = BopAMMSwapEncoder::new(Bytes::zero(20), Chain::Base, None);
