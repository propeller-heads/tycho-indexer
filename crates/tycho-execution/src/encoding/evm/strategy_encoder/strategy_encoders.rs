...
fn encode_swap_groups(
    ...
    for (index, group) in grouped_swaps.iter().enumerate() {
        let requires_ordered_quotes = swap_encoder_registry
            .get_encoder(&group.protocol_system)
            .is_some_and(|encoder| encoder.quote_behavior() == QuoteBehavior::BlockingOrdered);
    ...
    let mut encoded_groups: Vec<(usize, EncodedSwapGroup)> = encoded_tasks
        .into_iter()
        .flatten()
        .collect();
    encoded_groups.sort_by_key(|(index, _)| *index);
    Ok(encoded_groups
        .into_iter()
...
mod tests {
    ...
    mod single {
        ...
        /// The single-swap strategy passes the solution's quote attribution to the quote
        /// request, like the sequential and split strategies.
        #[test]
        fn test_single_swap_attributes_quotes() {
            ...
            let solution = Solution::new(
                ...
                BigUint::from(1_000u64),
                BigUint::from(900u64),
                vec![delayed_hashflow_swap(usdc, weth, Duration::ZERO, request_log.clone())],
            )
            ...
            assert_eq!(
                requests[0].1,
                solution.quote_attribution(),
                "single strategy passes the quote attribution"
            );
            assert_ne!(requests[0].1, sender, "attribution is derived, not the plain sender");
        }
...
(uint32,address,uint256,uint256,bytes),bytes)"
        ...
        /// The sequential strategy passes the solution's quote attribution to every hop's
        /// quote request. The derivation itself is covered by `models::tests`.
        #[test]
        fn test_sequential_swap_attributes_quotes() {
            let usdc = Bytes::from_str("0xa0b86991c6218b36c1d19d4a2e9eb0ce3606eb48").unwrap();
            let weth = weth();
            let dai = Bytes::from_str("0x6b175474e89094c44da98b954eedeac495271d0f").unwrap();
            let request_log: Arc<Mutex<Vec<(Bytes, Bytes)>>> = Arc::default();
            let solution = Solution::new(
                ...
                vec![
                    ...
                    delayed_hashflow_swap(weth, dai, Duration::ZERO, request_log.clone()),
                ],
            )
            .with_quote_request_id("quote-1".to_string());
            let encoder =
            ...
            assert_eq!(
                attributions,
                vec![solution.quote_attribution(), solution.quote_attribution()],
                "every hop carries the solution's quote attribution"
            );
        }
    }
