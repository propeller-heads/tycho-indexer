...
pub mod module {
    ...
    pub mod kind_store {
        ...
        pub enum UpdatePolicy {
            ...
            SetIfNotExists = 2,
            /// Provides a store where you can `add_*()` keys, where two stores merge by summing
            /// its values.
            Add = 3,
            /// Provides a store where you can `min_*()` keys, where two stores merge by leaving
            /// the minimum value.
            Min = 4,
            /// Provides a store where you can `max_*()` keys, where two stores merge by leaving
            /// the maximum value.
            Max = 5,
            /// Provides a store where you can `append()` keys, where two stores merge by
            /// concatenating the bytes in order.
            Append = 6,
        }
        impl UpdatePolicy {
