...
- **Cross-language integration tests**: Rust encoding tests call `write_calldata_to_file(test_identifier, hex_calldata)` (`src/encoding/evm/utils.rs`), which appends `name:hex` lines to `foundry/test/assets/calldata.txt`. Solidity tests then read that file via `loadCallDataFromFile(testName)` (`foundry/test/TestUtils.sol`) and execute the calldata against a mainnet fork. This verifies that Rust-encoded calldata is valid and executes correctly end-to-end.
