...
#[test]
fn test_single_encoding_strategy_curve_transfer_from_and_protocol_will_debit() {
    // Test TransferFromAndProtocolWillDebit with Curve where funds are transferred from
    // user's wallet to the router, then Curve pulls funds from the router.
    // Single swap: DAI (from wallet) -> (Curve TriPool) -> USDC
    //
    // This tests the case where:
    // 1. User has DAI in their wallet (not vault)
    // 2. First swap uses TransferFrom to move funds to router
    // 3. Curve uses ProtocolWillDebit to pull funds from router
