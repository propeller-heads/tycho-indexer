use std::{collections::HashMap, default::Default, iter::IntoIterator, time::Duration};
...
impl EthereumRpcClient {
    pub fn new(rpc_url: &str) -> Result<Self, RPCError> {
        ...
        Ok(Self { inner: rpc, batching, retry_policy })
    }

    pub fn with_batching(self, batching: Option<BatchingConfig>) -> Self {
        Self { inner: self.inner, batching, retry_policy: self.retry_policy }
    }

    pub fn with_retry_policy(self, retry_policy: RetryPolicy) -> Self {
        Self { inner: self.inner, batching: self.batching, retry_policy }
    }
...
mod tests {
    ...
    impl TestFixture {
        pub(crate) fn create_rpc_client(&self, batching: bool) -> EthereumRpcClient {
            ...
            EthereumRpcClient { inner: self.inner_rpc.clone(), batching, retry_policy }
        }
    }
